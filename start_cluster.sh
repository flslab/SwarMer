...
for (( i=1; i<num_of_total_servers; i++ )); do
    ...
    ssh -oStrictHostKeyChecking=no -f "${server_addr}" "ulimit -n 9999 && cd SwarMer && cp experiments/test_config$1.py test_config.py && sleep 10 && nohup python3 server.py ${num_of_total_servers} ${i} ${now} ${port} > my$1.log 2>&1 &" &
done
